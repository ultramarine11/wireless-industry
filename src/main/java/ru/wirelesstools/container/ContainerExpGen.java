...
public class ContainerExpGen extends Container {
	 ...
	 * this.tileentity.getFluidTank().getFluidAmount());
	 * System.out.println("container craft gui amount " +
	 * this.tileentity.getFluidTank().getFluidAmount());
	 * 
	 * 
	 * }
	...
	public void detectAndSendChanges() {
			...
			for (String name : getNetworkedFields()) {
				for (Object crafter : this.crafters) {
					if (crafter instanceof EntityPlayerMP) {
						...
						((NetworkManager) IC2.network.get()).updateTileEntityFieldTo(this.tileentity, name,
		...
		// icrafting.sendProgressBarUpdate(this, 2,
		// this.tileentity.getFluidTank().getFluidAmount());
		// System.out.println("detect send changes " +
		// this.tileentity.getFluidTank().getFluidAmount());
