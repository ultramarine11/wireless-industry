		...
		if (!world.isRemote) {
			if (!player.capabilities.isCreativeMode) {
				...
				MinecraftServer.getServer().getConfigurationManager()
						.sendChatMsg(
								new ChatComponentTranslation(
										EnumChatFormatting.DARK_RED + player.getGameProfile().getName() + " "
												+ StatCollector
														.translateToLocal("chat.server.message.used.pedestal.creative"),
										new Object[0]));
			} else {
				...
				MinecraftServer.getServer().getConfigurationManager()
						.sendChatMsg(
								new ChatComponentTranslation(
										EnumChatFormatting.YELLOW + player.getGameProfile().getName() + " "
												+ StatCollector
														.translateToLocal("chat.server.message.used.pedestal.survival"),
										new Object[0]));
			}
